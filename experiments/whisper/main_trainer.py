...
def main(config):
    ...
    if config["pretrained"]:
        ...
        # shorten the model embeddings to make it accept the input size (80, 100)
        # https://github.com/huggingface/transformers/issues/25744
        state_dict = model.state_dict()
        state_dict["encoder.embed_positions.weight"] = state_dict[
            "encoder.embed_positions.weight"
        ][:50, :]
        config = WhisperConfig.from_pretrained(HG_MODEL_PATH, max_source_positions=50)
        model = AutoModelForAudioClassification.from_config(config)
        model.load_state_dict(state_dict)
    else:
        config = AutoConfig.from_pretrained(HG_MODEL_PATH, max_source_positions=50)
